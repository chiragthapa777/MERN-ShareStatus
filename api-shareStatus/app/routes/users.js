...
router.get("/getsingleuser/:id", authenticate, async (req, res) => {
  try {
...
router.put("/updatefollow/:id", authenticate, async (req, res) => {
  try {
    ...
    if (meUser.following.includes(req.params.id)) {
      //unfollowing
      let newfollowedby = friendUser.followedBy.filter(
        (item) => item !== req.user.id
      );
      let newfollowing = meUser.following.filter(
        (item) => item !== req.params.id
      );
      friendUser = await User.findByIdAndUpdate(
        req.params.id,
        { followedBy: [...newfollowedby] },
        { new: true }
      );
      meUser = await User.findByIdAndUpdate(
        req.user.id,
      ...
      res
        .status(200)
        .json({
          message: "user unfollowed is successfully",
          meUser,
          friendUser,
        });
    } else {
      ...
      friendUser = await User.findByIdAndUpdate(
        req.params.id,
        { followedBy: [...friendUser.followedBy, req.user.id] },
        { new: true }
      );
      meUser = await User.findByIdAndUpdate(
        req.user.id,
        { following: [...meUser.following, req.params.id] },
        { new: true }
      );
      res
        .status(200)
        .json({ message: "user followed is successfully", meUser, friendUser });
    }
    //updating the Post
  } catch (error) {
    res.status(500).json({ error: error });
